...
export default function QuestionCard({ content, onRate }: Props) {
  ...
  return (
    <div class="flex flex-col items-center justify-center min-h-0 flex-1 w-full px-4">
      <div class="w-full max-w-md bg-white rounded-2xl shadow-lg border border-gray-100 flex flex-col">
        <div class="p-6 flex-1 flex flex-col items-center min-h-[280px]">
          ...
          <h2 class="text-lg font-semibold text-gray-800 text-center mb-4">{content.question}</h2>
          ...
          <div class="w-full space-y-2">
            {content.options.map((opt, idx) => {
              ...
              return (
                <button key={idx} onClick={() => toggleOption(idx)} class={cls}>
                  <span class="font-mono text-xs mr-2 opacity-60">{String.fromCharCode(65 + idx)}.</span>
                  {opt}
                </button>
              )
            })}
          ...
          {!submitted && selected.length > 0 && (
            <button
              ...
              class="mt-4 w-full py-3 rounded-xl bg-blue-500 text-white font-semibold shadow-md active:bg-blue-600 active:scale-[0.98] transition-all"
            >
              提交答案
            </button>
          )}
          ...
          {submitted && (
            <div class={`mt-4 text-center ${isCorrect ? 'text-green-600' : 'text-red-600'} font-semibold`}>
              {isCorrect ? '回答正确！' : '回答错误'}
              {content.explanation && (
                <p class="text-sm font-normal text-gray-500 mt-1">{content.explanation}</p>
              )}
            </div>
          )}
        </div>
